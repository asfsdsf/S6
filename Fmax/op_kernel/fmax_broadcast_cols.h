...
__aicore__ inline uint16_t dup_byte(uint8_t a) {
    return (static_cast<uint16_t>(a) << 8) | a;   // 0x00AB -> 0xABAB
}

constexpr uint32_t BufferNum_broadcast_cols = 1;
// other张量始终是需要广播的张量,只考虑了二维列广播
// 先考虑W很大的情况
template <typename T>
class KernelFmax_broadcast_cols {
public:
    __aicore__ inline KernelFmax_broadcast_cols() {}
    __aicore__ inline void Init(GM_ADDR input, GM_ADDR other, GM_ADDR out, uint32_t smallSize, uint32_t incSize, uint32_t formerNum, uint32_t totalSize,
        ...
        if (beginIndex + size > H) {// 列中最后一块
            this->size = H - beginIndex;
        }
        inputGm.SetGlobalBuffer((__gm__ T *)input + beginIndex * W, totalSize);// beginIndex是行的偏移
    ...
    __aicore__ inline void Process_iter(uint32_t offset, uint32_t outputIdxOffset) {
        ...
        if constexpr (std::is_same<T, int8_t>::value || std::is_same<T, uint8_t>::value || std::is_same<T, bool>::value) {
            T tmp_val = otherGm.GetValue(offset);
            Duplicate(otherLocal.template ReinterpretCast<uint16_t>(), dup_byte(static_cast<uint8_t>(tmp_val)), iterSize / 2 + 1);
        } else if constexpr (std::is_same<T, int64_t>::value) {
            return;
        } else {
            // uint16_t/int16_t/half/uint32_t/int32_t/float/bfloat16_t
            Duplicate(otherLocal, otherGm.GetValue(offset), iterSize);
        }
        ...
        for (uint32_t i = 0; i < smallLoopTimes; ++i) {
            ...
            // printf("1 otherLocal[0]=%f, otherLocal[1]=%f, otherLocal[2]=%f, otherLocal[3]=%f\n", otherLocal(0), otherLocal(1), otherLocal(2), otherLocal(3));
            // printf("outputIdxOffset=%d, outputIdx=%d\n", outputIdxOffset, outputIdx);
            // 计算
