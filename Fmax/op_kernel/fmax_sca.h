...
template <typename T>
class KernelFmax_sca {
public:
    __aicore__ inline KernelFmax_sca() {}
    __aicore__ inline void Init(GM_ADDR input, GM_ADDR other, GM_ADDR out, uint32_t smallSize, uint32_t incSize, uint32_t formerNum, uint32_t totalSize,
        ...
        if constexpr (std::is_same<T, int8_t>::value || std::is_same<T, uint8_t>::value || std::is_same<T, bfloat16_t>::value) {
            ...
            pipe->InitBuffer(inputBuf, BufferNum_sca, spaceSize * 2);
    ...
    __aicore__ inline int mapIndex(int outputIdx, uint32_t *inputShape, uint32_t *outputShape, const int nDims) {
        int inputIdx = 0;      // 输入张量的线性下标
        int stride = 1;        // 输入张量的步长累积
        int remainingIdx = outputIdx;  // 剩余待分解的输出下标
        for (int dim = nDims - 1; dim >= 0; --dim) {// 从尾向头处理
            int coord = remainingIdx % outputShape[dim];  // 当前维度坐标
    ...
    __aicore__ inline void Process_iter(uint32_t offset, uint32_t iterSize) {
        ...
        if (otherSize != totalSize) {
            CopyInBroadcastSca(otherGm, otherLocal, offset, iterSize, mmOtherDims, mmOutputDims, nOutputDims);
        } else {
